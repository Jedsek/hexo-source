...
---
> 让我们使用Gtk, 创建一个窗口, 了解下大致的框架, 作为旅途的起点吧
<!-- more -->
...
我们实际上还要添加一个名为 activate 的信号量(Signal), 你得像下面这样写:  
...
因为我想逐步地讲解, 对上面的东西来点小小的解释  

Gtk应用的理念是: App只关心在特定的时候需要做什么事  
什么时候做 ? Gtk已经帮我们安排好了, 至于 `事情的内容`, 则交由开发者自己决定  

接口方面, Gtk已经帮我们分类好一些虚函数, 它们也被称为 `信号(Signal)`, 我们只需要实现这些虚函数即可  
你可以将虚函数, 简单理解为, 其函数体是空的, 库作者想让你用, 但具体怎么用随你, 需要自己来填充函数体  
它们的调用顺序已经被安排好了, 只是需要等待我们实现函数体而已  
(也就是事件, 与事件发生时进行处理的回调函数)  
...
当某个App试图打开第二个窗口, Gtk全局系统, 会发送Signal(activate/open)给第一个窗口  
所有的初始化工作都应在startup中完成, 哪怕是第二个窗口相关的初始化  

这些只是Gtk给我们的虚函数中最为常见的几个, 实际上还有很多信号, 帮你自定义设计App  
本节后面, 会再讲下关于按钮被点击时的信号量, 帮你加深印象, 此处暂先略过  
...
我们得调用 `connect_activate`, 并传入一个函数作为参数  
根据定义, 这个被传入的函数, 具有静态的生命周期, 参数的类型是 `&Application`  
...
之前说过, 我们可以覆写的虚函数/信号量, 远远不止那四个, 这里为btn连接了名叫clicked的信号量  

再次加深一下印象: 
库的作者为你预先定义好了很多虚函数, 什么时候调用已经定好了, 但调用之后会做什么, 则自己决定  
这里, 按钮被按下这个事件发生后, 调用了对应的函数, 本来这应该是个虚函数  
但我们覆写了它, 被点击后, 会将按钮上的信息(Label)设置为 `Hello World!`  

clicked这个事情发生后, 本来就能啥也不做, 因此即使你不调用connect_clicked, 也不会出现警告  
